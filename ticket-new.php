    ...
    <div class="mt-6 space-y-5">
      <div>
        ...
        <input name="subject" required value="<?= e($_POST['subject'] ?? '') ?>" placeholder="Short summary of the problem"
               class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/25">
      </div>

      <div>
        <label class="mb-1.5 block text-sm text-slate-600">Department</label>
        <select name="department_id" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-teal-500">
      ...
      <div>
        ...
        <textarea name="body" rows="8" required placeholder="What happened? What did you expect? Any steps to reproduce?"
                  class="w-full resize-y rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/25"><?= e($_POST['body'] ?? '') ?></textarea>
      </div>
      ...
      <div>
        ...
        <input type="file" name="files[]" multiple
               class="w-full rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-500 file:mr-4 file:rounded-lg file:border-0 file:bg-teal-600 file:px-4 file:py-2 file:text-sm file:font-medium file:text-white hover:file:bg-teal-700">
      </div>
    </div>

    <div class="mt-7 flex gap-3">
      <button class="rounded-xl bg-teal-600 px-5 py-3 text-sm font-semibold text-white hover:bg-teal-700">Submit ticket</button>
